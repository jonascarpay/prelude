...
use prelude::algebra::{
    abstract_::{additive::zero, ring::one, Curve, Functor},
    geometric::complex::Complex,
    polynomial::{linear::remap2, quadratic::Quadratic},
    v2, V2,
};
...
fn main() {
    ...
    let remap = remap2(zero()..DIMS.map(|u| u as R), v2(-1., -1.)..v2(1., 1.));

    let r1 = c(0.5, 0.5);
    let r2 = c(-0.5, -0.5);
    let poly = Quadratic::from_roots(one(), r1, r2).over_ring();
